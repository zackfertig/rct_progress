...
The Release body includes direct links for Windows, macOS (.app droplet and CLI), and Linux (CLI and AppImage) for the current tag.
...
- macOS `.app` droplet
  - Download `RCT1-to-OpenRCT2-Droplet-macos-<arch>.zip`; extract `RCT1 → OpenRCT2 Droplet.app` and drag files onto it.
  - First‑run security: right‑click the app → Open to bypass Gatekeeper; or remove quarantine via Terminal: `xattr -dr com.apple.quarantine "RCT1 → OpenRCT2 Droplet.app"`.
